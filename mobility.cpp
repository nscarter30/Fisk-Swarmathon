...
void mobilityStateMachine(const ros::TimerEvent&) {
		...
		switch(stateMachineState) {
			...
			case STATE_MACHINE_TRANSFORM: {
				...
				//If no targets have been detected, assign a new goal
                    ...
                    goalLocation.x = currentLocation.x + (0.25 * cos(goalLocation.theta));
                    goalLocation.y = currentLocation.y + (0.25 * sin(goalLocation.theta));
				}
 ...
 setVelocity(double linearVel, double angularVel)
{
...
void targetHandler(const apriltags_ros::AprilTagDetectionArray::ConstPtr& message) {
	...
	if (message->detections.size() > 0) {
        ...
        if (hypot(hypot(tagPose.pose.position.x, tagPose.pose.position.y), tagPose.pose.position.z) < 0.1) {
            void openFingers();
            void lowerWrist();
            void closeFingers();
            void raiseWrist();
